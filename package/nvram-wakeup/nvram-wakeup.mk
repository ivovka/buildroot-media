...
define NVRAM_WAKEUP_BUILD_CMDS
    $(TARGET_MAKE_ENV) \
    $(MAKE) -C $(@D) CC="$(TARGET_CC)" SIZE="$(TARGET_CROSS)size" $(TARGET_MAKE_ENV)
endef

define NVRAM_WAKEUP_INSTALL_TARGET_CMDS
    $(TARGET_MAKE_ENV) \
    $(MAKE) prefix=$(TARGET_DIR)/usr -C $(@D) CC="$(TARGET_CC)" SIZE="$(TARGET_CROSS)size" install
...
define NVRAM_WAKEUP_BUILD_OPKG_CMDS
    $(TARGET_MAKE_ENV) \
    $(MAKE) prefix=$(BUILD_DIR_OPKG)/nvram-wakeup-$(NVRAM_WAKEUP_VERSION)/usr -C $(@D) CC="$(TARGET_CC)" SIZE="$(TARGET_CROSS)size" install
    ...
    rm -rf $(BUILD_DIR_OPKG)/$(NVRAM_WAKEUP_BASE_NAME)/usr/share
    rm -rf $(BUILD_DIR_OPKG)/$(NVRAM_WAKEUP_BASE_NAME)/usr/sbin
    rm $(BUILD_DIR_OPKG)/$(NVRAM_WAKEUP_BASE_NAME)/usr/bin/vdrshutdown
endef
