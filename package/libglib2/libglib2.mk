...
LIBGLIB2_PRIORITY = optional
LIBGLIB2_MAINTAINER = Vladimir Ivakin vladimir_iva@pisem.net
LIBGLIB2_DESCRIPTION = C support library
LIBGLIB2_OPKG_DEPENDENCIES = pcre,libffi
...
LIBGLIB2_DEPENDENCIES = host-pkg-config host-libglib2 zlib pcre $(if $(BR2_NEEDS_GETTEXT),gettext libintl)

LIBGLIB2_OPKG_DEPENDENCIES = zlib,pcre
...
define LIBGLIB2_REMOVE_DEV_FILES_OPKG
	rm -rf $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/lib/glib-2.0
	rm -rf $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/share/glib-2.0/gettext
	rmdir --ignore-fail-on-non-empty $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/share/glib-2.0
	rm -f $(addprefix $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/bin/,glib-genmarshal glib-gettextize glib-mkenums gobject-query gtester gtester-report)
endef
...
define LIBGLIB2_REMOVE_GDB_FILES_OPKG
	rm -rf $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/share/glib-2.0/gdb
	rmdir --ignore-fail-on-non-empty $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/share/glib-2.0
endef
...
define LIBGLIB2_REMOVE_LOCALE_OPKG
	rm -rf $(BUILD_DIR_OPKG)/libglib2-$(LIBGLIB2_VERSION)/usr/share/locale
endef

LIBGLIB2_PRE_BUILD_OPKG_HOOKS += LIBGLIB2_REMOVE_LOCALE_OPKG
