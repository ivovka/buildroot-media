...
DROPBEAR_BUILD_OPKG = YES
DROPBEAR_SECTION = net
DROPBEAR_PRIORITY = important
DROPBEAR_MAINTAINER = Vladimir Ivakin vladimir_iva@pisem.net
DROPBEAR_DESCRIPTION = A small SSH 2 server designed for small memory environments.
...
define DROPBEAR_BUILD_OPKG_CMDS
	mkdir -p $(BUILD_DIR_OPKG)/dropbear-$(DROPBEAR_VERSION)/usr/sbin
	mkdir -p $(BUILD_DIR_OPKG)/dropbear-$(DROPBEAR_VERSION)/usr/bin
	mkdir -p $(BUILD_DIR_OPKG)/dropbear-$(DROPBEAR_VERSION)/etc/init.d
	$(INSTALL) -m 755 $(@D)/dropbearmulti $(BUILD_DIR_OPKG)/dropbear-$(DROPBEAR_VERSION)/usr/sbin/dropbear
	for f in $(DROPBEAR_TARGET_BINS); do \
		ln -snf ../sbin/dropbear $(BUILD_DIR_OPKG)/dropbear-$(DROPBEAR_VERSION)/usr/bin/$$f ; \
	done
	$(INSTALL) -m 0755 -D package/dropbear/S50dropbear $(BUILD_DIR_OPKG)/dropbear-$(DROPBEAR_VERSION)/etc/init.d/S50dropbear
endef
