...
gdb_target-opkg: $(GDB_TARGET_DIR)/gdb/gdb
	...
	echo "Maintainer: Vladimir Ivakin vladimir_iva@pisem.net" >> $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION)/CONTROL/control
	echo -e "Description: Debugger" >> $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION)/CONTROL/control
	echo "Section: devel" >> $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION)/CONTROL/control
	echo "Priority: optional" >> $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION)/CONTROL/control
	echo "Source: $(GDB_SOURCE)" >> $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION)/CONTROL/control
	echo "Depends: ncurses" >> $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION)/CONTROL/control
	scripts/opkg-build -c -o 0 -g 0 -O $(BUILD_DIR_OPKG)/gdb-$(GDB_VERSION) $(BINARIES_DIR_OPKG)

gdb_target: ncurses $(TARGET_DIR)/usr/bin/gdb gdb_target-opkg
