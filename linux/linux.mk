...
LINUX_BUILD_OPKG = YES
LINUX_SECTION = system
LINUX_PRIORITY = required
LINUX_DESCRIPTION = Linux kernel

LINUX_MAKE_FLAGS_CMN = \
	HOSTCC="$(HOSTCC)" \
...
LINUX_MAKE_FLAGS = $(LINUX_MAKE_FLAGS_CMN) INSTALL_MOD_PATH=$(TARGET_DIR)
LINUX_MAKE_FLAGS_OPKG = $(LINUX_MAKE_FLAGS_CMN) INSTALL_MOD_PATH=$(BUILD_DIR_OPKG)/linux-$(LINUX_VERSION)
...
define LINUX_BUILD_OPKG_CMDS
	mkdir -p $(BUILD_DIR_OPKG)/linux-$(LINUX_VERSION)
	install -m 0644 -D $(LINUX_IMAGE_PATH) $(BUILD_DIR_OPKG)/linux-$(LINUX_VERSION)/$(LINUX_IMAGE_NAME)
	@if grep -q "CONFIG_MODULES=y" $(@D)/.config; then 	\
		$(TARGET_MAKE_ENV) $(MAKE1) $(LINUX_MAKE_FLAGS_OPKG) -C $(@D) 		\
			DEPMOD="$(HOST_DIR)/usr/sbin/depmod" modules_install ;		\
		rm -f $(BUILD_DIR_OPKG)/linux-$(LINUX_VERSION)/lib/modules/$(LINUX_VERSION_PROBED)/build ; \
		rm -f $(BUILD_DIR_OPKG)/linux-$(LINUX_VERSION)/lib/modules/$(LINUX_VERSION_PROBED)/source ; \
	fi
endef
